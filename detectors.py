...
from detect_utils import Detector, Hook, get_scores

class Baseline(Detector):
    """ [ICLR'17] Hendrycks & Gimpel, A baseline for detecting misclassified and out-of-distribution examples in neural networks.
    ...
    def __init__(self, model, num_classes, **kwargs):
        ...
        for m_name, m in model._modules.items():
            if isinstance(m, nn.Linear):
                self.softmax_layer = Hook(m, nn.Softmax(dim=1))
...
class Odin(Detector):
    """ [ICLR'18] Liang et al., Enhancing the reliability of out-of-distribution image de-tection in neural networks.
    The OOD score is defined as negative max. of softmax with the temperature scaling and input perturbation.
    """

    def __init__(self, model, num_classes, normalizer=(1.0, 1.0, 1.0), **kwargs):
...
class Mahalanobis(Detector):
    """ [NeurIPS'18] Lee et al., A simple unified framework for detecting out-of-distribution samples and adversarial attacks.
    ...
    def __init__(self, model, num_classes, net_type='', normalizer=(1.0, 1.0, 1.0), **kwargs):
        ...
        for m_name, m in model._modules.items():
            if net_type == 'densenet':
                if m_name in ['conv1', 'trans1', 'trans2', 'relu']:
                    self.feature_hooks.append(Hook(m))
            elif net_type == 'resnet':
                if m_name == 'bn1':
                    self.feature_hooks.append(Hook(m, nn.ReLU(inplace=False)))
        ...
        if len(self.feature_hooks) > 5:
            self.feature_hooks = [self.feature_hooks[i] for i in np.linspace(0, len(self.feature_hooks)-1, 5, endpoint=True).astype(int)]
    ...
    def tune_parameters(self, valid_loader, out_valid_loader, num_samples=1000):
        ...
        for magnitude in M_list:
            ...
            X_train = np.concatenate((X_in[:num_train], X_out[:num_train]), axis=0)
            Y_train = np.concatenate((np.zeros(num_train), np.ones(num_train)))
            regressor = LogisticRegressionCV(n_jobs=-1).fit(X_train, Y_train)
            
            X_valid_in = regressor.predict_proba(X_in[num_train:num_samples])[:, 1]
            X_valid_out = regressor.predict_proba(X_out[num_train:num_samples])[:, 1]
    ...
    def _get_ood_scores(self, x):
        ...
        for i in range(self.num_features):
            ...
            for c in range(self.num_classes):
                ...
                md = torch.mm(torch.mm(centered, Variable(self.precision[i])), centered.t()).diag()
    ...
    def _fit_Mahalanobis(self, train_loader):
        ...
        for data, target in train_loader:
            ...
            for i, out_features in enumerate(out_layers):
                ...
                gap = out_features.mean(dim=2)
                for c in range(self.num_classes):
                    feature_vecs[i][c].append(gap[torch.nonzero(target==c, as_tuple=False).squeeze(1)].cpu().numpy())
        ...
        for i in range(self.num_features):
            ...
            tied_cov = EmpiricalCovariance(store_precision=True, assume_centered=False)
            tied_cov.fit(X)
    ...
    def _set_feature_extractor(self, x):
        if isinstance(x, nn.Conv2d):
            self.feature_hooks.append(Hook(x))
