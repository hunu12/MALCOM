...
import ncd.cuda_utils as cutils
import ncd.transforms as tutils
import detectors.baselines
...
class CCP(nn.Module):
    ...
    def forward(self, x, i):
        ...
        _, strings = (
            out.unsqueeze(2) - self.levels[i].unsqueeze(0).unsqueeze(3)
        ).abs().min(2)
        ...
        ncds = torch.empty((
            ...
            self.pmaps[i].size(0),
            self.pmaps[i].size(1)
        )).cuda().type(torch.float32).contiguous()
...
class Malcom(detectors.baselines.Mahalanobis):
    ...
    def tune_parameters(self, valid_loader, out_valid_loader, num_samples=1000):
        ...
        X_in = detectors.detect_utils.get_scores(
            self, valid_loader, num_samples
        )
        X_out = detectors.detect_utils.get_scores(
            self, out_valid_loader, num_samples
        )
    ...
    def _get_ood_scores(self, x):
        ...
        for i, out_features in enumerate(out_layers):
            ...
            if self.ood_tuning:
                ...
                for c in range(self.num_classes):
                    ...
                    md = torch.mm(
                        torch.mm(centered, self.precision[i]), centered.t()
                    ).diag()
        ...
        for c in range(self.num_classes):
            ...
            md = torch.mm(
                torch.mm(centered, self.precision), centered.t()
            ).diag()
            distance_from_mean.append(md)
    ...
    def _fit_linearization(self, scan='hilbert'):
        if scan == 'hilbert':
            self.curves = tutils.hilbert_curves(self.feature_list)
        elif scan == 'vertical':
            self.curves = tutils.vertical_curves(self.feature_list)
        else:
            self.curves = tutils.horizontal_curves(self.feature_list)
    ...
    def _fit_quantization(self, train_loader, num_levels=4, tol=1e-1, max_iter=10):
        ...
        for i, (num_maps, _, _) in enumerate(self.feature_list):
            ...
            self.levels[i] = tutils.lloyd_max_quantizer(
                out_features.double(),
                num_levels=num_levels, tol=tol, max_iter=500
            ).double()
        ...
        for t in range(max_iter):
            ...
            for data, _ in train_loader:
                ...
                for i, (num_maps, H, W) in enumerate(self.feature_list):
                    out_features = out_layers[i].permute((1, 0, 2, 3))\
                        .reshape(num_maps, -1).double()
                
                    temp_errors, temp_indices = (
                        out_features.unsqueeze(2) - self.levels[i].unsqueeze(1)
                    ).pow(2).min(2)
                    new_levels[i] = new_levels[i].scatter_add_(
                        1, temp_indices, out_features
                    )
                    normalizer[i] = normalizer[i].scatter_add_(
                        1, temp_indices, 
                        torch.ones_like(temp_indices, dtype=torch.double)
                    )
            ...
            if old_error is None:
                ...
                print("\t\tquantization iter {}: relative error {:.3f}"\
                    .format(t, rel_error))
            old_error = sum_error
    ...
    def _fit_prototypical_maps(self, train_loader):
        symbol_list = [np.zeros((num_maps, self.num_levels))
                       for num_maps, _, _ in self.feature_list]
        mean_maps = []
        ...
        for data, _ in train_loader:
            ...
            for i, out_features in enumerate(out_layers):
                ...
                _, strings = (
                    out_features.unsqueeze(2) - \
                    self.levels[i].unsqueeze(0).unsqueeze(3)
                ).abs().min(2)
        ...
        for i in range(self.num_features):
            ...
            symbol_list[i] = (symbol_list[i] * mean_maps[i].size(2)) \
                .cumsum(axis=1).round().astype(int)[:, :-1]
            ...
            temp_threshold = torch.gather(
                sorted_values, 1, torch.LongTensor(symbol_list[i]).cuda()
            ).unsqueeze(2) ##
            ...
            for l in range(self.num_levels - 1):
                strings += (
                    mean_maps[i] > 
                    temp_threshold[:, l].unsqueeze(0).repeat(
                        (mean_maps[i].size(0), 1, 1)
                    )
                ).type(torch.uint8)
    ...
    def _fit_Mahalanobis(self, train_loader):
        ...
        for data, target in train_loader:
            ...
            for i, out_features in enumerate(out_layers):
                ...
                if self.ood_tuning:
                    for c in range(self.num_classes):
                        indices = torch.nonzero(target==c, as_tuple=False).squeeze(1)
                        feature_vecs[i][c].append(twofold_poolings[indices].cpu().numpy())
                else:
            ...
            if not self.ood_tuning:
                ...
                for c in range(self.num_classes):
                    indices = torch.nonzero(target==c, as_tuple=False).squeeze(1)
                    feature_vecs[c].append(temp_feature_vecs[indices].cpu().numpy())
